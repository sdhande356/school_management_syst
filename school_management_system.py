...
from tkcalendar import DateEntry  # pip install tkcalendar To get the user to enter a date.
import sqlite3 

# Creating the universal font variables
headlabelfont = ("Noto Sans CJK TC", 15, 'bold') #Font, size and style
...
def reset_form(): 
    ...
    connector.commit() ##it will save the changes
    reset_fields() 
...
def add_record():
    ...
    if not name or not email or not contact or not gender or not DOB or not stream:
        ...
        try:
            ...
            connector.commit()
            mb.showinfo('Record added', f"Record of {name} was successfully added") #popup
            reset_fields()  
            display_records()  
        except:
...
def remove_record(): 
    if not tree.selection():
        ...
        connector.execute('DELETE FROM SCHOOL_MANAGEMENT WHERE STUDENT_ID=%d' % selection[0]) #dynamic selection
        connector.commit()
...
def view_record(): 
    ...
    values = tree.item(current_item) 
    selection = values["values"] 
    print(selection[5])##since dob is at 5th index
    date = datetime.date(int(selection[5][:4]), int(selection[5][5:7]), int(selection[5][8:])) #2022-10-19
    print(date)
...
# main.resizable(0, 0)  #cannot be resized

# Creating the background and foreground color variables
lf_bg = 'SteelBlue' # bg color for the left_frame
cf_bg = 'blue' # bg color for the center_frame
...
Label(main, text="SCHOOL MANAGEMENT SYSTEM", font=headlabelfont, bg='SkyBlue').pack(side=TOP, fill=X) #fill completely along x axis
...
right_frame = Frame(main, bg="Gray-35")
...
#columns we are getting in right frame trrview is wuidget
tree = ttk.Treeview(right_frame, height=100, columns=('Student ID', "Name", "Email Address", "Contact Number", "Gender", "Date of Birth", "Stream"))
...
# decide the text that will be displayed on the table’s first row as column headers
...
# how much space we need to give to all those columns.
...
main.update() #used to show the updated screen.
main.mainloop()